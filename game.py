...
player_img = pygame.transform.scale(asset_data.load_surface(asset_data.PLAYER_GIF), (32, 32))
enemy_img = pygame.transform.scale(asset_data.load_surface(asset_data.ENEMY_GIF), (32, 32))
ground_tex = asset_data.load_surface(asset_data.GROUND_BMP).convert()
...
def reset_game():
    ...
    player = Creature(60, 60, characters[char_index]['color'],
                      characters[char_index]['hp'], player_img)
    enemies = [Enemy(random.randint(100, selected_size-100),
                    random.randint(100, selected_size-100), enemy_img)
               for _ in range(5)]
    walls = generate_walls(selected_size)
    captured.clear()
    particles.clear()

help_text = ["ASDW 로 이동", "j 키: 공격", "k 키: 포획", "l 키: 소환"]
...
while running:
                ...
                if evt.key == pygame.K_k:
                    for e in enemies:
                        if pygame.Vector2(e.x-player.x, e.y-player.y).length() < 50 and e.hp <= 1:
                            ...
                            break
                elif evt.key == pygame.K_l and captured:
    ...
    if state == TITLE:
        ...
        tex_w, tex_h = ground_tex.get_width(), ground_tex.get_height()
        for x in range(0, selected_size, tex_w):
            for y in range(0, selected_size, tex_h):
                screen.blit(ground_tex, (x, y))
        for w in walls:
            pygame.draw.rect(screen, (120,120,120), w)
            pygame.draw.line(screen, (60,60,60), w.topleft, w.topright)
            pygame.draw.line(screen, (60,60,60), w.topleft, w.bottomleft)
        for e in enemies:
            e.draw(screen)
        player.draw(screen)
